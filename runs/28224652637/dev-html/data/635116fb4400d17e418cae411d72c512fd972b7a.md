...
- Name: homepage.spec.ts >> logout link exist test @smoke
...
```ts
  ...
  7  | });
  8  | 
  9  | test('home page title test @smoke', async ({ homePage }) => {
  ...
  14 | 
  15 | 
  16 | test('logout link exist test @smoke', async ({ homePage }) => {
> 17 |     expect(await homePage.isLogoutLinkExist()).toBeTruthy();
     |                                                ^ Error: expect(received).toBeTruthy()
  18 | });
  19 | 
  20 | 
  21 | test('home page headers exist test', async ({ homePage }) => {
